...
function subAddresses(
	...
	const responseBody = uniqueAddresses
		.map((address) => {
			...
			const 协议类型 = atob(啥啥啥_写的这是啥啊);
			const vlessLink = `${协议类型}://${UUID}@${address}:${port}?encryption=none&security=tls&sni=${伪装域名}&fp=chrome&alpn=${encodeURIComponent(ALPN)}&type=ws&host=${伪装域名}&path=${encodeURIComponent(最终路径)}#${encodeURIComponent(addressid + 节点备注)}`;
